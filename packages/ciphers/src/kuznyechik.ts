...
function linearValue(block: Bytes): number {
  ...
  for (let index = 0; index < KUZNYECHIK_BLOCK_SIZE; index += 1) {
    value ^= multiply(KAPPA[index]!, block[index]!);
  }
  return value;
}

function rTransform(block: Bytes): Bytes {
  const output = new Uint8Array(KUZNYECHIK_BLOCK_SIZE);
  output[0] = linearValue(block);
  output.set(block.subarray(0, KUZNYECHIK_BLOCK_SIZE - 1), 1);
  return output;
}

function rInverse(block: Bytes): Bytes {
  let last = multiply(KAPPA[KUZNYECHIK_BLOCK_SIZE - 1]!, block[0]!);
  for (let index = 0; index < KUZNYECHIK_BLOCK_SIZE - 1; index += 1) {
    last ^= multiply(KAPPA[index]!, block[index + 1]!);
  }
  const output = new Uint8Array(KUZNYECHIK_BLOCK_SIZE);
  output.set(block.subarray(1), 0);
  output[KUZNYECHIK_BLOCK_SIZE - 1] = last;
  return output;
}

function lTransform(block: Bytes): Bytes {
  let output: Bytes = Uint8Array.from(block);
  for (let index = 0; index < KUZNYECHIK_BLOCK_SIZE; index += 1) output = rTransform(output);
  return output;
}

function lInverse(block: Bytes): Bytes {
  let output: Bytes = Uint8Array.from(block);
  for (let index = 0; index < KUZNYECHIK_BLOCK_SIZE; index += 1) output = rInverse(output);
  return output;
}
...
function applyTable(input: Bytes, table: Uint32Array, key?: Bytes): Bytes {
  const output = new Uint8Array(KUZNYECHIK_BLOCK_SIZE);
  applyTableInto(output, input, table, key);
  return output;
}
...
function lsx(block: Bytes, key: Bytes): Bytes {
  return applyTable(block, LSX_TABLE, key);
}

function iterationConstant(index: number): Bytes {
  const vector = new Uint8Array(KUZNYECHIK_BLOCK_SIZE);
  vector[KUZNYECHIK_BLOCK_SIZE - 1] = index;
...
export class KuznyechikCipher implements BlockCipher {
  ...
  public constructor(key: Bytes) {
    ...
    this.#decryptKeys = Array.from({ length: 8 }, (_, index) => lInverse(this.#roundKeys[8 - index]!));
  }
  ...
  public decryptBlock(block: Bytes): Bytes {
    ...
    state = applyTable(substitute(state, PI), DECRYPT_TABLE);
  ...
  #expandKey(key: Bytes): readonly Bytes[] {
    ...
    for (let group = 0; group < 4; group += 1) {
      for (let round = 1; round <= 8; round += 1) {
        const transformed = lsx(left, iterationConstant(group * 8 + round));
        const next = xor(transformed, right);
        right = left;
        left = next;
      }
