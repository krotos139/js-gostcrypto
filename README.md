...
notes. Runnable mini-projects for Node.js, React, AngularJS, jQuery, and a
bundler-free browser are collected in [examples](examples/README.md).
...
Measured on September 14, 2026, with Node.js 22.14.0 for Windows x64 on an
...
|---|---:|---:|---:|
| Streebog-256, 8 KiB | **12.53 MB/s** | 0.27 MB/s | 11.60 MB/s |
| Streebog-512, 8 KiB | **13.35 MB/s** | 0.27 MB/s | 12.89 MB/s |
| Kuznyechik, one block | **881 ns** | 76.9 us | 69.7 us¹ |
| Magma, one block | **161 ns** | 2.2 us | 1.1 us¹ |
| Kuznyechik key setup | **6.2 us** | 262.8 us | 70.2 us¹ |
| Sign, 256 bit | **417.8 us** | 955.4 us | 768.2 us² |
| Verify, 256 bit | **3.58 ms** | 4.87 ms | 7.71 ms² |
| Sign, 512 bit | **977.0 us** | 3.26 ms | 1.81 ms² |
| Verify, 512 bit | **15.13 ms** | 20.32 ms | 37.70 ms² |
...
as a correctness smoke test. It also builds the bilingual MkDocs site in strict
mode and publishes it to GitHub Pages from `main` or `master`. Tagged `v*`
releases use the separate npm release workflow with provenance.

## Documentation

The documentation is available in [English](docs/en/index.md) and
[Russian](docs/ru/index.md). Build the same strict site used by CI locally:
...
For a live preview, run `npm run docs:serve`. The published site is configured
for [GitHub Pages](https://krotos139.github.io/js-gostcrypto/).
