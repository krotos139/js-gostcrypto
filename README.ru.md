...
Готовые мини-проекты для Node.js, React, AngularJS, jQuery и браузера без
сборщика собраны в каталоге [examples](examples/README.md).
...
Измерено 14 сентября 2026 года в Node.js 22.14.0 для Windows x64 на Intel Core
...
|---|---:|---:|---:|
| Стрибог-256, 8 КиБ | **12.53 MB/s** | 0.27 MB/s | 11.60 MB/s |
| Стрибог-512, 8 КиБ | **13.35 MB/s** | 0.27 MB/s | 12.89 MB/s |
| Кузнечик, один блок | **881 нс** | 76.9 мкс | 69.7 мкс¹ |
| Магма, один блок | **161 нс** | 2.2 мкс | 1.1 мкс¹ |
| Развёртка ключа Кузнечика | **6.2 мкс** | 262.8 мкс | 70.2 мкс¹ |
| Подпись, 256 бит | **417.8 мкс** | 955.4 мкс | 768.2 мкс² |
| Проверка, 256 бит | **3.58 мс** | 4.87 мс | 7.71 мс² |
| Подпись, 512 бит | **977.0 мкс** | 3.26 мс | 1.81 мс² |
| Проверка, 512 бит | **15.13 мс** | 20.32 мс | 37.70 мс² |
...
проверку корректности. CI также строго собирает двуязычный сайт MkDocs и
публикует его в GitHub Pages из ветки `main` или `master`. Теги `v*`
обрабатываются отдельным workflow публикации в npm с provenance.

## Документация

Документация доступна на [английском](docs/en/index.md) и
[русском](docs/ru/index.md). Локально та же строгая сборка, что и в CI,
запускается так:
...
Для просмотра с автоматическим обновлением используйте `npm run docs:serve`.
Публикуемый сайт настроен для
[GitHub Pages](https://krotos139.github.io/js-gostcrypto/).
